...
class Module {
    ...
    public function get_support_form( $seller_id = "" ) {
		...
		$name = get_user_meta($current_userid, 'first_name', true).' '.get_user_meta($current_userid, 'last_name', true);
        ...
        ob_start();
        ?>  
        <div class="dokan-support-intro-user"><strong><?php printf( __( 'Hi, %s', 'dokan' ), $name ) ?></strong></div>
        ...
        <form class="dokan-form-container" id="dokan-support-form">
            ...
            <div class="dokan-form-group">
                <?php if ( !empty( $customer_orders ) ) :?>
                            <select class="dokan-form-control dokan-select" name="order_id" required>
                                <option value=""><?php _e( 'Select Order ID', 'dokan' ); ?></option>
        ...
        <?php }else{ 
			...
			echo ' <div class="dokan-support-noorder-text">Opps.. <br>You should make an order first from this Store.</div>';
