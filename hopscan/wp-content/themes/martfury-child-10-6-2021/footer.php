...
<script>
    ...
    jQuery(document).ready(function(){
        ...
        (function(){
            if(jQuery('#billing_country').val() == 'RW' || jQuery('#shipping_country').val() == 'RW') 
            {
                jQuery('.page-id-3534 #billing_city').val('citybilling');
                jQuery('.page-id-3534 #billing_state').val('statebilling');
...
if (strpos($_SERVER['REQUEST_URI'], "my-account/?popup=register") !== false){ ?> 
 <script>
    ...
    dffd('.showlogin').attr('href','https://ewt.webdesigntexas.us/hopscan/my-account/');
 });
 </script>
<?php } ?>
 <script>
...
jQuery(document).ready(function(){
    ...
    jQuery('.customeditlnk').click(function()
    {
        ...
        if(jQuery('#shipping_country').val() != 'RW')
        {
            jQuery('#shipping_postcode').val('');
        }
        else
        {
            jQuery('.page-id-3416 #shipping_city').val('citybilling');
            jQuery('.page-id-3416 #shipping_state').val('statebilling');
        }
