...
$rs_billing_address_1 = get_user_meta($current_user->ID, 'rs_billing_address_1', true);
$rs_billing_city = get_user_meta($current_user->ID, 'rs_billing_city', true);
$rs_billing_state = get_user_meta($current_user->ID, 'rs_billing_state', true);
$rs_billing_postcode = get_user_meta($current_user->ID, 'rs_billing_postcode', true);
$billing_phone = get_user_meta($current_user->ID, 'billing_phone', true);

?>
<?php if(empty($rs_billing_address_1) && empty($rs_billing_city) && empty($rs_billing_state) && empty($billing_phone) && empty($rs_billing_postcode) || (empty($rs_billing_address_1) || empty($billing_phone)) ) { ?>
	<div class="alert alert-danger empty_message">
	  You have to complete all required details in <a href="<?php echo site_url('/').'my-account/edit-account/'; ?>"><strong>Account Details</strong> </a> before you can become a Driver or a vendor.
	</div>
<?php } /*if (empty($rs_billing_address_1) || empty($billing_phone) ) { ?>
	<div class="alert alert-danger empty_message">
	  To complete registration process please fill all the required detail here <a href="<?php echo site_url('/').'my-account/edit-account/'; ?>"><strong>Account Details</strong></a>
	</div>
<?php } */?>	
<p>
	<?php
...
<ul class="dokan-account-migration-lists">
    <li>
        <div class="dokan-w8 left-content">
            ...
            <p><strong>HoPscan</strong> Drivers earn money delivering products to the recipients from <strong>HoPscan</strong> Registerd Vendors.</p>
        </div>
