...
// Get country code for vendor
jQuery( "#dokan_address_country" ).change(function() {
   var siteurl = WP_SITE_URLS.siteurl;
  jQuery('#setting_phone').val('');
  var selected_country = jQuery(this).find(":selected").val();
		...
		jQuery.ajax({
				...
				if(response.status) {
						jQuery('#setting_phone').val(response.countrycode);
						jQuery(".loading_icn").css('display', 'none');
                }
               } 
       });
...
jQuery(document).ready(function(){
  ...
  var selected_country = jQuery("#shipping_country").find(":selected").val();
...
// State dropdown initial level

var siteurl = WP_SITE_URLS.siteurl;
  //jQuery('#shipping_phone_code').val('');
  var selected_country = jQuery("#country").find(":selected").val();
  //alert(selected_country);
		jQuery.ajax({
				...
				if(response) {
				        ...
				        jQuery('#dokan_address_state').append('<option value="">Select State</option>');
				        for(var key in response){
				            
				            jQuery('#dokan_address_state').append('<option value="">Select State</option><option value="'+response[key]+'">'+response[key]+'</option>');
                            //jQuery('#dokan_address_state').append('<option value="'+response[key]+'">'+response[key]+'</option>');
                        }
                }
               } 
       });
...
jQuery( "#country" ).change(function() {
  ...
  //alert(selected_country);
		jQuery.ajax({
				...
				if(response) {
				        //console.log(response);
				        jQuery('#dokan_address_state').html('');
				        for(var key in response){
				            
				            jQuery('#dokan_address_state').append('<option value="'+response[key]+'">'+response[key]+'</option>');
                            //jQuery('#dokan_address_state').append('<option value="'+response[key]+'">'+response[key]+'</option>');
                            jQuery("#dokan_address_state option[value='Select State']").attr('value', '');
                        }
                }
               } 
       });
...
var siteurl = WP_SITE_URLS.siteurl;
  //jQuery('#shipping_phone_code').val('');
  var selected_country = jQuery("#dokan_address_country").find(":selected").val();
  //alert(selected_country);
		jQuery.ajax({
                type: 'POST',
                dataType: "json",
...
var siteurl = WP_SITE_URLS.siteurl;
  //jQuery('#shipping_phone_code').val('');
  var selected_country = jQuery("#drvr_country").find(":selected").val();
  //alert(selected_country);
		jQuery.ajax({
                type: 'POST',
                dataType: "json",
...
// Driver registration frontend state dropdown

var siteurl = WP_SITE_URLS.siteurl;
  //jQuery('#shipping_phone_code').val('');
  var selected_country = jQuery('.wpuf_country_4113').find(":selected").val();
  //alert(selected_country);
		jQuery.ajax({
                type: 'POST',
                dataType: "json",
				...
				if(response) {
				        //console.log(response);
				        jQuery('.wpuf_state_4113').html('');
				        for(var key in response){
				            
				            jQuery('.wpuf_state_4113').append('<option value="'+response[key]+'">'+response[key]+'</option>');
                            //jQuery('#dokan_address_state').append('<option value="'+response[key]+'">'+response[key]+'</option>');
                            jQuery(".wpuf_state_4113 option[value='Select State']").attr('value', '');
                        }
                }
               } 
       });
jQuery( ".wpuf_country_4113" ).change(function() {
   var siteurl = WP_SITE_URLS.siteurl;
  //jQuery('#shipping_phone_code').val('');
  var selected_country = jQuery(this).find(":selected").val();
  //alert(selected_country);
				  ...
				  success: function(data) {
					...
					jQuery('span.dokan-loading').remove();
					  if(data == 'success'){
						...
						var full_url = templateUrl + '/my-account/driver-dashboard/';  
...
jQuery(document).ready(function() {
    jQuery('#phone-otp').hide();
    jQuery('.verification_msg').hide();
    jQuery('#verify_top').hide();
    jQuery( "#ph_otp" ).click(function() {
       var siteurl = WP_SITE_URLS.siteurl;
       var phone_code = jQuery('#phone-cd').val();
       var phone_number = jQuery('#phone').val();
    		jQuery.ajax({
                    type: 'POST',
                    dataType: "json",
                    url :  siteurl,
    				...
    				if(response) {
    				    ...
    				    jQuery('#phone-otp').show();
    				    jQuery('.verification_msg').show();
    				    jQuery('#verify_top').show();
    				    jQuery('#ph_otp').val('Resend Code');
                    }
                   } 
           });
    });
...
jQuery(document).ready(function() {
    jQuery('#phone-otp1').hide();
    jQuery('.verification_msg1').hide();
    jQuery('#verify_top1').hide();
    jQuery('#ph_otp').hide();
    
    jQuery( "#ph_otp1" ).click(function() {
       var siteurl = WP_SITE_URLS.siteurl;
       var username = jQuery('#user_login').val();
    		jQuery.ajax({
    				...
    				if(response) {
    				    console.log(response);
    				    localStorage.setItem('otp_forgot', response);
    				    jQuery('#phone-otp1').show();
    				    jQuery('.verification_msg1').show();
    				    jQuery('#verify_top1').show();
    				    jQuery('#ph_otp1').val('Resend Code');
                    }
                   } 
           });
    });
    ...
    var cemail = jQuery('#confirm_email').val();
    var country = jQuery('#my_country_field').val();
    var phone = jQuery('#phone').val();
    if(fname != '' && lname != '' && email != '' && cemail != '' && country != '' && phone != ''){
        jQuery('#ph_otp').show();
    }
    }
    else{
