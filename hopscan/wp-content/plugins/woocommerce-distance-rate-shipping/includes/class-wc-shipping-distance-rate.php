...
if ( ! class_exists( 'WC_Shipping_Distance_Rate' ) ) {
	...
	class WC_Shipping_Distance_Rate extends WC_Shipping_Method {
		...
		public function __construct( $instance_id = 0 ) {
			...
			/*$this->id           = 'pickup_local';
			$this->instance_id  = absint( $instance_id );
			$this->method_title = __( 'Pickup', 'woocommerce-distance-rate-shipping' );
			$this->supports     = array(
				'shipping-zones',
				'instance-settings',
				'settings',
		...
		private function set_settings() {
			...
			$dokan_settings = get_user_meta( $post_author, 'dokan_profile_settings', true );

			$address1 	= $shop_info[address][street_1];
			$address2 	= $shop_info[address][street_2];
			$zip 		= $shop_info[address][zip];
			$city 		= $shop_info[address][city];
			$state 		= $shop_info[address][state];
			$country 	= $shop_info[address][country];
			...
			if(!empty($dokan_settings['distance_unit'])){
			$this->unit          = $dokan_settings['distance_unit'];
			}else{
			...
			if(!empty($address1) && !empty($state)  && !empty($country)  && !empty($zip) ){
					
				$this->address_1     = $address1;
				$this->address_2     = $address2;
				$this->city          = $city;
				$this->postal_code   = $zip;
				$this->state_country = $state;
		...
		public function get_rates( $package = array() ) {
			...
			if(!empty($address1) && !empty($state)  && !empty($country)  && !empty($zip) ){
				$full_address = $address1.' '.$address2.' '.$city.' '.$state.' '.$zip.', '.$country;
			}else{
				$full_address = $this->get_shipping_address_string();
				}
			
			$distance = $this->get_api()->get_distance( $full_address, $this->get_customer_address_string( $package ), false, $this->mode, $this->avoid, $this->unit, $region );
			...
			if ( $this->show_distance && is_object( $distance ) ) {
				...
				$pieces = explode(" ", $distance->rows[0]->elements[0]->distance->text);
				if($pieces[0]<3){
					/*$label_suffix[] = $distance->rows[0]->elements[0]->distance->text .', <span class="nt_tx">Flat Rate 6$ upto 3km</span>';*/

					$label_suffix[] = $distance->rows[0]->elements[0]->distance->text .'';	
				}else{
					
					/*$label_suffix[] = $distance->rows[0]->elements[0]->distance->text.', <span class="nt_tx">Flate Rate 6$ <br/> Additional Distance surcharge<br/> ( '.$pieces[0].'-3 )*1.1 </span>';*/

					$label_suffix[] = $distance->rows[0]->elements[0]->distance->text.'';

				}
				
								//$label_suffix[] = $distance->rows[0]->elements[0]->distance->value;
				//$label_suffix[] = $pieces[0]*1.1+6;
			}
			...
			//$dokan_settings['dd_range']
			}

			foreach ( $this->rules as $rule ) {
				
				
				if(isset($dokan_settings['dd_range'])){
						
						foreach($rule as $key => $val){
								if($dokan_settings['dd_range'] && $dokan_settings['cost_per_dis']){
									if($key == 'max'){
										$rule['max'] = $dokan_settings['dd_range'];
									}else if($key == 'cost_unit'){
										$rule['cost_unit'] = $dokan_settings['cost_per_dis'];
										}
								}
							}
			...
			//if($dokan_settings['dd_range'])
			/*if(is_object( $distance )){
				$pieces = explode(" ", $distance->rows[0]->elements[0]->distance->text);
				echo '<pre> Distance is';
				print_r($pieces);
				echo '</pre>';
			}else{
				echo 'No distance OBject';
				}
			*/
			
			if($distance <= $dokan_settings['local_range']){
			$rates[] = array(
					'id'    => 'local_pc:11',
