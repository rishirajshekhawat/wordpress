...
function martfury_child_enqueue_scripts() {
	...
	if (strpos($_SERVER['REQUEST_URI'], "/my-account/account-migration/") !== false){
	    wp_dequeue_script('google-map');
	    wp_dequeue_script('dokan-maps');
	    wp_dequeue_script('google-maps');
	}
...
wp_register_script('custom-script', get_stylesheet_directory_uri() . '/assets/js/custom-script.js', array('jquery'),time(), true);
...
function user_register_function($user_id) {
	...
	if(isset($_POST['phone-cd']) && isset($_POST['phone'])){
		update_user_meta($user_id, 'billing_phone_code', $_POST['phone-cd']);
...
if($is_store_featured)
{
	...
	<h2 class="store_cat">Featured Partners</h2>
	<?php
		$users1[]=$seller->ID;
	}
	else{
...
if($user_con['address']['country'] == $currentcountry){
  ...
  $args = array(
	...
	<h2 class="store_cat"><?php echo $attbs['cat']; ?></h2>
	<?php } }
    $html .='<div id="dokan-seller-listing-wrap" class="grid-view">
                <div class="seller-listing-content">
...
function logout_user_register_form(){
	...
	<div class="woocommerce-form-register-toggle">
		...
		New customer? <a href="https://hopscan-stg.com/my-account/?popup=register" class="">Create a new account</a>	</div>
	</div>
...
function misha_no_email_validation( $fields ){
    $fields['shipping']['rw_state']['required'] = false;
    $fields['shipping']['rw_city']['required'] = false;
    return $fields;
} 
*/
        ...
        (function($){
            ...
            $( 'form.checkout' ).on( 'change', '#shipping_country', function() {
                ...
                if( location == 'RW' ) {
	                ...
	                $('input#full_address_for_rwanda').val('');
...
function checkout_billing_state_field( $checkout_fields ) {
       ...
       unset($checkout_fields['billing']['rw_state']);
       
       unset($checkout_fields['billing']['rw_city']);
...
function get_lat_long($address){
    ...
    $region = '';
    $json = file_get_contents("https://maps.google.com/maps/api/geocode/json?address=$address&sensor=false&region=Rwanda&key=AIzaSyBEynUhUUPcC2u8AWtZHLkbnNoS0M5qwP0");
...
function custom_shipping_fields_checkout( $fields ) {
	...
	$fields['rw_state'] = array(
        'label'     => __('State/Province', 'woocommerce'),
	    'placeholder'   => _x('', 'placeholder', 'woocommerce'),
	    'required'  => false,
	    'clear'     => true,
		'class'        => array('form-row-wide','showstateinput1'),
	    'type'      => 'select',//end of options
		'priority'     => 85,
		'options'     => array(
		'' => __('Select State/Province', 'woocommerce' ),
        'Kigali Province' => __('Kigali Province', 'woocommerce' ),
        'Northern Province' => __('Northern Province', 'woocommerce' ),
		'Southern Province' => __('Southern Province', 'woocommerce' ),
		'Eastern Province' => __('Eastern Province', 'woocommerce' ),
		'Western Province' => __('Western Province', 'woocommerce' )
		
        )
	);	
	$fields['rw_city'] = array(
        'label'     => __('City/District', 'woocommerce'),
	    'placeholder'   => _x('', 'placeholder', 'woocommerce'),
	    'required'  => false,
	    'clear'     => true,
		'priority'     => 85,
		'class'        => array('form-row-wide','cityshow1' ),
	    'type'      => 'select',//end of options
		'options'     => array(
		'' => __('Select City/District', 'woocommerce' )		
        )
	);
...
// console.log(jQuery('#full_address_for_rwanda').val());
...
<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBEynUhUUPcC2u8AWtZHLkbnNoS0M5qwP0&libraries=places&callback=initAutocomplete"></script>
    ...
    if ( ! class_exists( 'WC_Your_Shipping_Method' ) ) {
        ...
        class WC_flat_Shipping_Method extends WC_Shipping_Method {
            ...
            public function calculate_shipping( $package ) {
                ...
                $store_country1  = $store_info['address']['country'];
                ...
                $address_user_country = $result['shipping_country'];
                ...
                if($store_named1 == $address_user && $address_user_country == $store_country1 && $address_user_country == 'RW'){
                // Register the rate
                $this->add_rate( $rate );
                }elseif($address_user_country != $store_country1){
                
                    unset( $rates['local_pc:11'] );
...
function add_flat_shipping_methods($rates){
    ...
    $address_user_blank = $result['full_address_for_rwanda'];
    $address_user_country = $result['shipping_country'];
    ...
    if($result['shipping_address_1'] == 'no-shipping')
    {
        $rates = array();
    }

    if(empty($address_user_blank) && $store_country1 == 'RW')
    {
        $rates = array();
    }
    
    if($address_user_country != $store_country1){
        $rates = array();
    }
...
function send_message_to_driver(){
$order_id_id = $_POST['oredr_id']; 
$driver_id_id = $_POST['driver_id']; 

$phone1 = get_user_meta($driver_id_id,'billing_phone_code',true);
$phone2 = get_user_meta($driver_id_id,'billing_phone',true);
$phone12 = $phone1.$phone2;

$order = wc_get_order( $order_id_id );
$seller = dokan_get_seller_id_by_order( $order_id_id );
...
$args = array( 
	'number_to' => $phone12,
	'message' => '[HoPscan]-You have been assigned order #'.$order_id_id.' by seller: '.$store_name.' / '.$address1.'| '.$address2.' Phone: '.$store_phone.'|', 
); 
$sms = twl_send_sms($args);
}
add_action( 'wp_ajax_send_message_to_driver', 'send_message_to_driver'); 
add_action( 'wp_ajax_nopriv_send_message_to_driver', 'send_message_to_driver'); 
...
function custom_billing_fields( $fields ) {
    ...
    if(is_page(3534)) {
        ...
        $fields['rw_state']['required'] = false;
        $fields['rw_city']['required'] = false;
    }
    return $fields;
}
...
function custom_send_email_notifications( $order_id, $old_status, $new_status, $order ){
    ...
    if ( $new_status == 'cancelled' ) {
        ...
        $wc_emails['WC_Email_Cancelled_Order']->recipient = $vendor_email;
    ...
    elseif ( $new_status == 'failed' ) {
        ...
        $wc_emails['WC_Email_failed_Order']->recipient = $vendor_email;
        // Sending the email from this instance
...
function bbloomer_remove_product_from_cart_programmatically() {
   ...
   global $woocommerce;
    ...
    if($author_id != $author_id_old && $count > 0){
        $product_cart_id = WC()->cart->generate_cart_id( $product_id );
        $cart_item_key = WC()->cart->find_product_in_cart( $product_cart_id );
        if ( $cart_item_key ) WC()->cart->remove_cart_item( $cart_item_key );
    }
    $count++;
    $author_id_old = get_post_field('post_author', $product_id);
    }
   }
}
...
function action_woocommerce_after_checkout_validation( $data, $error ) {
    if ( $data['shipping_country'] == 'RW' && empty( $data['rw_state'] ) ) {
        $error->add( 'validation', '<b>Shipping State/Province</b> is a required field.' );
    }
}
    ...
    <script>
        (function($) {
            $(document).ready(function () {
                ...
                $( '#shipping_country' ).change( required_or_optional );

                function required_or_optional() {
                    if ( $( '#shipping_country' ).val() == 'RW' ) {
                        // Required
                        $( '#rw_state' ).prop( 'required', true );
                        $( 'label[for="rw_state"] .optional' ).remove();
                        $( 'label[for="rw_state"]' ).append( '<abbr class="required" title="required">*</abbr>' );
                    } else {
                        $( '#rw_state' ).removeProp( 'required' );
                        $( 'label[for="rw_state"] .required' ).remove();

                        // Avoid append this multiple times
                        if ( $( 'label[for="rw_state"] .optional' ).length == 0 ) {
                            $( 'label[for="rw_state"]' ).append( '<span class="optional">(optional)</span>' );
                        }
...
// Validate
function action_woocommerce_after_checkout_validation1( $data, $error ) {
    if ( $data['shipping_country'] == 'RW' && empty( $data['rw_city'] ) ) {
        $error->add( 'validation', '<b>Shipping City/District</b> is a required field.' );
    }
}
add_action('woocommerce_after_checkout_validation', 'action_woocommerce_after_checkout_validation1', 10, 2 );

// jQuery
function action_woocommerce_after_order_notes1( $checkout ) {
    ?>
    <script>
        (function($) {
            $(document).ready(function () {
                ...
                function required_or_optional() {
                    if ( $( '#shipping_country' ).val() == 'RW' ) {
                        // Required
                        $( '#rw_city' ).prop( 'required', true );
                        $( 'label[for="rw_city"] .optional' ).remove();
                        $( 'label[for="rw_city"]' ).append( '<abbr class="required" title="required">*</abbr>' );
                    } else {
                        $( '#rw_city' ).removeProp( 'required' );
                        $( 'label[for="rw_city"] .required' ).remove();

                        // Avoid append this multiple times
                        if ( $( 'label[for="rw_city"] .optional' ).length == 0 ) {
                            $( 'label[for="rw_city"]' ).append( '<span class="optional">(optional)</span>' );
                        }
...
add_action( 'woocommerce_after_order_notes', 'action_woocommerce_after_order_notes1', 10, 1 );

$cats =  get_categories(array('taxonomy' => 'store_category'));
//print_r($cats);
/**
...
function isa_dynamic_submenu_logout_link( $items, $args ) {
        ...
        $existing_menu_item_db_id = 5830;
        if ( $theme_location !== $args->theme_location ) {
            return $items;
        }
        
        $results = $wpdb->get_results( "SELECT * FROM wp_term_taxonomy WHERE taxonomy ='store_category'" );
        $i =1;
        foreach($results as $result){
            $count = $result->count;
            if($count>0){
            $id = $result->term_id;
            $categories = $wpdb->get_results( "SELECT * FROM wp_terms WHERE term_id ='$id'" );
            foreach($categories as $category){
                $name = $category->name;
                $url = 'https://hopscan-stg.com/store-listing/?store_categories='.$category->slug;
                $new_menu_item_db_id = rand(10000,99999);
