...
function dokan_validate_cart_for_single_seller_mode( $valid, $product_id ) {
    ...
    if ( count( $vendors ) > 1 ) {
        wc_add_notice( __( 'Sorry! You can only buy from one shop at a time.', 'dokan' ), 'error' );
        wc_add_notice( __( 'Please remove products that are not sold by your favorite shop from the current cart.', 'dokan' ), 'error' );
        wc_add_notice( __( 'Always make sure your cart only contains products from a single shop!', 'dokan' ), 'error' );
        $valid = false;
    }
