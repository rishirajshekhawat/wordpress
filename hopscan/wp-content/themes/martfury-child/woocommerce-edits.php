...
function send_otp_to_recipients($order_id) {
	...
	$address_sep = substr($address1, 0, strpos($address1, ','));
	...
	if($user_phone){
			...
			$args = array( 
    			...
    			'message' => '[HoPscan]-You have received an order #'.$order_id.' from '.$fname1.' '.$lname2.' | One Time Password: '.$otp.'|Seller: '.$store_name.' / '.$address_sep.' '.$address2.' '.$address3.' '.$address5.'|Phone:'.$store_phone.'|', 
     		); 
			} else{
			$args = array( 
    			'number_to' => $user_phone,
    			'message' => '[HoPscan]-You have received an order #'.$order_id.' from '.$fname1.' '.$lname2.' | One Time Password: '.$otp.'|Seller: '.$store_name.' / '.$address1.' '.$address2.' '.$address3.' '.$address4.' '.$address5.'|Phone No:'.$store_phone.'|', 
     		); 	
			} 
        	...
        	$args2 = array(
        			...
        			'message' => '[HoPscan]-You have received an order #'.$order_id.'',
         		); 
...
<p class="form-row form-row-wide" id="billing_company_field" data-priority="30">
	...
	$address_city    = get_user_meta( get_current_user_id(), 'rs_billing_city', true );
	$address_state   = get_user_meta( get_current_user_id(), 'rs_billing_state', true );
	// woocommerce_form_field(
		// 'rs_billing_state',
		// array(
			// 'type'        => 'text',
	...
	// );
	?>
	<p  id="dokan-states-box" class="woocommerce-form-row woocommerce-form-row--wide form-row form-row-wide hidestateinput">
        <label class="dokan-w3 control-label" for="dokan_address[state]"><?php esc_html_e( 'State ', 'dokan-lite' ); ?></label>
        <select name="rs_billing_state" class="dokan-form-control <?php echo esc_attr( $address_state_class ) ?>" id="dokan_address_state" required/>
            <option value="">Select State</option>
        </select>
    </p>
    <p  id="dokan-states-box" class="woocommerce-form-row woocommerce-form-row--wide form-row form-row-wide showstateinput">
            <label class="dokan-w3 control-label" for="dokan_address[state]"><?php esc_html_e( 'State/Province ', 'dokan-lite' ); ?></label>
            <select name="rs_billing_state" class="dokan-form-control <?php echo esc_attr( $address_state_class ) ?>" id="dokan_address_state1" required>
    ...
    <p class="woocommerce-form-row woocommerce-form-row--wide form-row form-row-wide">
                        ...
                        <input <?php echo esc_attr( $required_attr ); ?> <?php echo esc_attr( $disabled ) ?> id="dokan_address[city]" value="<?php echo esc_attr( $address_city ); ?>" name="rs_billing_city" placeholder="<?php esc_attr_e( 'Town / City' , 'dokan-lite' ) ?>" class="dokan-form-control input-md" type="text">
                        </p>
                        
					<p class="cityshow">
                        <select id="dokan_address[city]" name="rs_billing_city" class="city_d dokan-form-control input-md" required>
...
function misha_save_account_details( $user_id ) {
	...
	update_user_meta( $user_id, 'rw-state', sanitize_text_field( $_POST['rw_state'] ) );
	update_user_meta( $user_id, 'rw-city', sanitize_text_field( $_POST['rw_city'] ) );
