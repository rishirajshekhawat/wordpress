		...
		if ( 0 != $user_id ) {
			...
			if ( in_array( 'driver', $user_roles, true ) ) {
				...
				if ( isset( $order_id ) && ( '' != $order_id ) && ( $driver_id == $user_id ) ) {
					...
					$customer_phone = $recipuent_pcode.$recipuent_phone; 
					...
					echo '<p>';
						...
						if ( isset( $order_billing_phone ) ) {
							$phone_customer = '<a href="tel:' . esc_html( $customer_phone ) . '" class="button ddwc-button customer">' . esc_attr__( 'Call Recipients', 'ddwc' ) . '</a> ';
						}
					...
					echo '<table class="ddwc-dashboard 1">';
					echo '<tbody>';

					do_action( 'ddwc_driver_dashboard_order_details_table_tbody_top' );

					if ( '' !== $order_billing_fname ) {
						echo '<tr><td class="fontinc"><strong>' . esc_attr__( 'Customer', 'ddwc' ) . '</strong></td><td><strong>' . $order_billing_fname . ' ' . $order_billing_lname . '</strong></td></tr>';
					}	
						
					// Display customer.
					if ( '' !== $order_shipping_fname ) {
						echo '<tr><td class="fontinc"><strong>' . esc_attr__( 'Recipients', 'ddwc' ) . '</strong></td><td><strong>' . $order_shipping_fname . ' ' . $order_shipping_lname . '</strong></td></tr>';
					} 

					// Display payment method details.
					if ( isset( $payment_gateway ) && FALSE !== $payment_gateway ) {
						$payment_method = '<tr><td class="fontinc"><strong>' . esc_attr__( 'Payment method', 'ddwc' ) . '</strong></td><td>' . $payment_gateway->title . '</td></tr>';
						echo apply_filters( 'ddwc_driver_dashboard_payment_method', $payment_method );
					}

					// Display order date.
					if ( isset( $order_date_created ) ) {
						echo '<tr><td class="fontinc"><strong>' . esc_attr__( 'Order date', 'ddwc' ) . '</strong></td><td>' . esc_html( $order_date_created ) . ' - ' . esc_html( $order_time_created ) . '</td></tr>';
					}
					...
					echo '<table class="ddwc-dashboard 2">';
					...
					if ( $assigned_orders ) {
						...
						echo '<table class="ddwc-dashboard 3">';
						...
						echo '<table class="ddwc-dashboard 4">';
