    ...
    <section id="listing_detail_banner" class="parallex-bg">
        <div id="listing_img_slider">
            ...
            <?php 
				...
				echo "<div class='header_height'></div>";	
			  }
                ?>                           
            </div>   
        </div>
                    ...
                    <div class="olomo_detail">
                        <div class="pricing_info">
                            <?php echo olomo_price_dynesty(get_the_ID());
                             $leadForm = $olomo_options['lead_form_switch'];
                                  if($leadForm=="1"){?>   
                                    ...
                                    <?php
                                            ...
                                            foreach($buisness_hours as $key=>$value){
                                                ...
                                                if($day == $key){
                                                    ...
                                                    echo '<li class="today-timing clearfix"><span class="hours_title"><i class="fa fa-clock-o"></i>'.esc_html__($dayName).'</span>';
                                                        if($time > $open && $time < $close){
                                                            echo '<a class="Opened">'.esc_html__('Open Now','olomo').'</a>';
                                                        }else{
                                                            echo '<a class="closed">'.esc_html__('Closed Now!','olomo').'</a>';
                                                        }								
                                                    echo '<span>'.esc_html__($newTimeOpen).' - '.esc_html__($newTimeClose).'</span></li>';
                                                }
                                            }
                                            if(is_array($keyArray) && !in_array($day, $keyArray)){
                                                echo '<li class=""><span class="hours_title"><i class="fa fa-clock-o"></i>'.esc_html__($dayName).'</span>';
                                            ...
                                            foreach($buisness_hours as $key=>$value){
                                                ...
                                                $newTimeOpen = date('h:i A', $open);
                                                $newTimeClose = date('h:i A', $close);
                                                echo '<li><span class="hours_title"><i class="fa fa-clock-o"></i>'.esc_html__($dayName).'</span>';
                                                echo '<span>'.esc_html__($newTimeOpen).' - '.esc_html__($newTimeClose).'</span></li>';
                                            }
                                            echo '</ul>';
                                    ?>
      ...
      $user_id = get_current_user_id();
		...
		<?php } } ?>
                        ...
                        <div id="writereview">
                          ...
                          <?php setSinglePostViews(get_the_ID()); ?>
