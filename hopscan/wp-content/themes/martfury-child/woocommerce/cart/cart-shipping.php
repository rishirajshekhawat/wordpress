...
<tr class="woocommerce-shipping-totals shipping">
	<td colspan="2" data-title="<?php echo esc_attr( $package_name ); ?>" style="text-align: left;">
    
    <?php 
		$substr = 'Shipping';
		$attachment = ' from ';
		$newstring = str_replace($substr, $substr.$attachment, $package_name);
		$disp_name = substr($package_name, 11);
	...
	?>
     <h4 class="shipping-title"><?php echo wp_kses_post( $newstring ); ?></h4>
		...
		foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
			...
			$store_address    = dokan_get_seller_short_address( $author, false );
		?>
		<p class="vendor-address-checkout"><i class="fa fa-map-marker"></i> <?php echo $store_address; ?></p>
