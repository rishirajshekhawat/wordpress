...
function ddwc_driver_dashboard_change_status_forms() {
	...
	if ( 'driver-assigned' == $order_status ) {
		...
		$change_status .= '<input type="text" name="outfordeliverymessage" value="Order Picked UP" placeholder="' . esc_attr__( 'Add a note', 'ddwc' ) . '" class="ddwc-ofdmsg" disabled/>';
	...
	if ( 'out-for-delivery' == $order_status ) {
		$change_status  = '<h4>' . esc_attr__( 'Update Delivery Status', 'ddwc' ) . '</h4>';
		$change_status .= '<p><select id="order_option" class="dokan-form-control"><option value="" selected>Select Option</option><option value="order returned">Order Returned</option><option value="order completed">Order Completed</option></select></p>';
		
		$change_status .= '<form method="post" class="driver_updateorder" id="dr_updateorder">';
		$change_status .= '<p><strong>' . esc_attr__( 'Shipping note here.', 'ddwc' ) . '</strong></p>';
		$change_status .= '<input type="text" name="ordermessage" value="Order Returned" placeholder="' . esc_attr__( 'Add a note', 'ddwc' ) . '" class="ddwc-ofdmsg" readonly/>';
		...
		$change_status .= '<div class="otpmatch" style="display:none;"><p>Never release the order  without a valid OTP has beed provided!!</p><div class="otpmatch_section">
