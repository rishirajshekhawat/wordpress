...
class Alg_WC_CCF_Scripts {
	...
	function enqueue_admin_scripts() {
		if ( isset( $_GET['page'], $_GET['tab'], $_GET['section'] ) && 'wc-settings' == $_GET['page'] && 'alg_wc_ccf' == $_GET['tab'] && 'field_' == substr( $_GET['section'], 0, 6 ) ) {
	 ...
	 * @version 1.4.7
	 * @since   1.0.0
	 * @todo    [maybe] store `//ajax.googleapis.com/ajax/libs/jqueryui/1.9.0/themes/base/jquery-ui.css` etc. locally
	 */
	function enqueue_frontend_scripts() {
		...
		if ( $do_load['fees'] ) {
			...
			wp_localize_script( 'alg-wc-ccf-fees',
				'alg_wc_ccf_fees_fields',
				'input[name="' . implode( '"], input[name="', $fees_fields ) . '"]'
			);
		}
	}
