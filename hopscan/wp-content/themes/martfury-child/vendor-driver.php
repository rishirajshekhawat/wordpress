           ...
           <div style="clear:both;"></div>
            ...
            <form method="post" id="settings-form"  action="" class="dokan-form-horizontal" autocomplete="off" enctype="multipart/form-data"> 
				...
				<div  id="dokan-states-box" class="dokan-form-group hidestateinput">
					<label class="dokan-w3 dokan-control-label" for="dokan_address[state]"><?php esc_html_e( 'State ', 'dokan-lite' ); ?></label>
					<div class="dokan-w5">
					<select name="dr_state" Placeholder="State" class="dokan-form-control right_bl" id="dokan_address_state">
					<option value="">Select State</options>    
					</select>
					</div>
				</div>
				<div  id="dokan-states-box" class="dokan-form-group showstateinput">
						<label class="dokan-w3 dokan-control-label" for="dokan_address[state]"><?php esc_html_e( 'State/Province ', 'dokan-lite' ); ?></label>
						<div class="dokan-w5">
						<select name="dr_state" class="removename dokan-form-control <?php echo esc_attr( $address_state_class ) ?>" id="dokan_address_state1" required>
					...
					<div class="dokan-form-group" id="">
                        <label class="dokan-w3 dokan-control-label" for="dokan_address[city]"><?php esc_html_e( 'City/District', 'dokan-lite' ); ?>
                        </label>
                        <div class="dokan-w5 cityyhide">
                        <input <?php echo esc_attr( $required_attr ); ?> <?php echo esc_attr( $disabled ) ?> id="dokan_address[city]" value="-" name="dr_city" placeholder="<?php esc_attr_e( 'Town / City' , 'dokan-lite' ) ?>" class="dokan-form-control input-md" type="text">
                        </div>
						<div class="dokan-w5 cityshow">
							<select id="dokan_address[city]"  name="dr_city" class="city_d dokan-form-control input-md" required>
...
<div class="internal_drouter">
  ...
  foreach ( $drivers as $driver ) {
    ...
    $city = $address['rs_billing_city'][0];
    $state = $address['rs_billing_state'][0];
    $country = $address['billing_country'][0];
    $full_add = '';
    ...
    //     }
    // if(isset($street_2) && $address['country'][0] != 'RW' ){
    //     $full_add .= $street_2.' ';
    // }
    ...
    //     // }
    // if(isset($billing_city) && $address['country'][0] != 'RW' ){
    //     $full_add .= $billing_city.', ';
    // }
    
    // $billing_state = $address['rs_billing_state'][0];
    // // if(empty($address['rs_billing_state'][0]) && $address['billing_country'][0] != 'RW'){
    // //     $billing_state = $address['billing_state'][0];
    // //     }
    // if(isset($billing_state) && $address['country'][0] != 'RW' ){
    ...
    // //     }
    // if(isset($billing_country) && $address['billing_country'][0] != 'RW' ){
