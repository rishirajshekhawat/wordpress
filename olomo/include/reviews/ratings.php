...
/* ============== Ratings Stars ============ */if(!function_exists('olomo_ratings_stars')){
	function olomo_ratings_stars($metaboxID, $postID) {
		...
		while( $rating > 0){
			if($rating < 1){
				echo '<i class="fa fa-star star-worst"></i>';
				$rating--;
				$blankstars--;
			}
			else if($rating >=1 && $rating < 2){
				echo '<i class="fa fa-star star-bad"></i>';
				$rating--;
				$blankstars--;
			}
			else if($rating >=2 && $rating < 3.5){
				echo '<i class="fa fa-star star-satisfactory"></i>';
				$rating--;
				$blankstars--;
			}
			else if($rating >=3.5 && $rating <= 5){
				echo '<i class="fa fa-star star-good"></i>';
				$rating--;
				$blankstars--;
			}
		}
...
/* ============== Ratings Figure ============ */
if (!function_exists('olomo_ratings_figure')) {
	function olomo_ratings_figure($postID) {
		$print = '';
		$total = '';
...
/* ============== Set_listing_ratings ============ */
if (!function_exists('olomo_set_listing_ratings')) {
	function olomo_set_listing_ratings($review_id, $listing_id =null, $new_rating = null, $action) {		
...
/* ============== Get rating number =================== */
if(!function_exists('olomo_ratings_numbers')){
	function olomo_ratings_numbers($listing_id){
		$rating = 0;
		$reviews_ids = listing_get_metabox_by_ID('reviews_ids', $listing_id);
		$reviews = array();
		$reviews_ids = listing_get_metabox_by_ID('reviews_ids', $listing_id);
...
/* ============== Highest avaerage rating ============ */
if (!function_exists('olomo_highest_average_rating')) {
	function olomo_highest_average_rating($listing_id, $new_rate) {
...
/* ============== Total reviewes rating ============ */
if (!function_exists('olomo_total_reviews_add')) {
	function olomo_total_reviews_add($listing_id){
