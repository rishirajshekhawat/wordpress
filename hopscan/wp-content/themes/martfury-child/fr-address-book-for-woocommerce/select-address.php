	...
	?>
        <p class="form-row" id="<?php echo "fabfw_address_{$type}_id_field" ?>">
           <?php if($type == 'shipping'){?>
            ...
            <?php } else { ?>
            <label class="recap_ship"><?php esc_html_e('Add/Select a Recipient', 'fr-address-book-for-woocommerce') ?>
            <br><i style="color:red;font-size:14px">(<?php _e('You have reached to maximum limit of Recipients, you can not add more Recipient now.', 'fr-address-book-for-woocommerce') ?>)</i>
            </label>
            ...
            <select name="<?php echo "fabfw_address_{$type}_id" ?>" id="<?php echo "fabfw_address_{$type}_id_{$id}" ?>" class="slect_recipients"  data-placeholder="Select a recipient" data-input-classes="" tabindex="-1" aria-hidden="true" data-select2-id="<?php echo "select2-data-fabfw_address_{$type}_id" ?>">
                <?php if (count($addresses) < fr_address_book_for_woocommerce()->max_addresses) { ?>
				<option value="<?php echo '0'; ?>" selected>Add/Select a Recipient</option>
                <?php } else { ?>
                <option value="<?php echo '0'; ?>">Select a Recipient</option>
...
jQuery(document).ready(function($){
	...
	<?php if(count($addresses) > 0){ ?>
		...
		jQuery('.woocommerce-shipping-fields__field-wrapper').removeClass('hidden');
	<?php } ?>
