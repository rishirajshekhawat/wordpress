<article class="help-content-area">
        	<h1 style="margin-top: 0px; font-size: 20px;">Chat Support Tickets</h1>
          	<p></p>
...
if ( isset( $_POST['Submit'] ) ){
    ...
    $sql = $wpdb->prepare( "INSERT INTO chat_support (id, chat_id, user_id, vendor_id, topic,message) VALUES ( '','$chat_id','$user_id','$vendor_id','','$message' )");
    $wpdb->query($sql);
}
?>
    ...
    <thead>
        <tr>
            ...
            <!--<th>Action</th>--->
                ...
                $results = $wpdb->get_results( "SELECT * FROM `chat_support` WHERE `user_id` = '$userid'");
                //print_r($results);
                if(!empty($results))                       
                {    
                    foreach($results as $row){ ?> 
                    <tr>
                        <td><a href="http://ewt.webdesigntexas.us/hopscan/my-account/userchat/?chat_id=<?php echo $row->chat_id; ?>"><?php echo $row->chat_id; ?></a></td>
